...
<script>
	...
	export default {
		name: 'row',
			props: ['model','depth','columns','isdraggable','border', 'custom_field','onCheck','isContainChildren','dataLists'],
			data() {
				return {
					open: false,
					visibility: 'visible'
				...
				onCheckboxClick(evt, model) {
					...
					if (list && this.isContainChildren) {
						...
						if(this.isContainChildren){
							...
							if(j.length==0){
								this.$set(this.dataLists[i], 'checked', true)
							} else {
								this.$set(this.dataLists[i], 'checked', false)
							}
						}
					}
